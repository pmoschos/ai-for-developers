...
class PersonalityChatbot:
    ...
    def load_personality(self, name: str):
        ...
        ΔΟΣΜΕΝΟ — Δεν χρειάζεται αλλαγή.
        """
        personality = load_personality(name) # φορτώνει το personality από το αρχείο
        self.personality = personality

        #Ορισμός system propmt
    ...
    def chat(self, user_message: str) -> str:
        ...
        """
        #Καταχώρηση μηνύματος στην μνήμη
        self.conversation.add_message("user", user_message)

        #Open AI API Call
        response = client.chat.completions.create(
            model = 'gpt-4o-mini',
            messages = self.conversation.get_messages(), #φέρνει όλο το ιστορικό
            max_tokens=500,
            temperature=self.personality.get("temperature", 0.8) 
        )

        #Καταχώρηση απάντησης στην μνήμη
        assistant_message = response.choices[0].message.content
