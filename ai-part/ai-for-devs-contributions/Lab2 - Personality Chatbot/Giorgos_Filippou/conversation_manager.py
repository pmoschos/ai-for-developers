...
class ConversationManager:
    ...
    def add_message(self, role: str, content: str) -> None:
        ...
        if role in ('user', 'assistant'):
            ...
            if len(self.messages) > self.max_history:
                self.messages = self.messages[-self.max_history:] #Αφού max_history = 50, κρατάμε τα τελευταία 50 μηνύματα (50η θέση από το τέλος)
        else:
            raise ValueError(f"Invalide role: {role}")
    ...
    def get_messages(self) -> List[Dict[str, str]]:
        ...
        """
        result = []
        #Προσθήκη system prompt στο output
        if self.system_prompt: 
            result.append({'role':'system', 'content':self.system_prompt})
        
        #Προσθήκη υπόλοιπων μηνυμάτων στο output
    ...
    def get_summary(self) -> Dict:
        ...
        TODO: Χρησιμοποιήστε sum() + generator expression για μέτρηση
              π.χ. sum(1 fm in self.messages if m.role == "user")
        """
        return {
            'message_count': sum(1 for msg in self.messages), 
    ...
    def export_conversation(self, filepath: str = None) -> List[Dict]:
        ...
        """
        # Μετατροπή Ιστορικού μηνυμάτων σε JSON
        data = [{
            "role": msg.role,
            "content": msg.content,
            "timestamp": msg.timestamp.isoformat()
